...
public class Sighting_Mapview extends SherlockFragmentActivity implements OnClickListener
{
    ...
    private void setUpMapIfNeeded()
    {
        if (mMap == null)
        {
            mMap = ((SupportMapFragment) getSupportFragmentManager().findFragmentById(R.id.mapview_fullscreen))
                    .getMap();
            if (mMap != null)
            {
            ...
            case MAPTYPE:
                return new AlertDialog.Builder(this)
                        .setItems(R.array.maptype_array, new DialogInterface.OnClickListener()
                        {
                            ...
                            {
                                float
                                        zoomLevel;    //collect zoom level info, then switch map types then reset zoom essentially
                                if (item == 0)
                                {
